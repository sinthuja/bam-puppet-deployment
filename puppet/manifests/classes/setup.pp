	...
	define clean ( $mode, $target ) {	
		...
		exec { "remove_${name}_poop":
        	        ...
        	        command         => $mode ? {
                        	                ...
                        	                "fresh" => "kill -9 `cat ${target}/wso2carbon.pid` ; rm -rf ${target} ; rm -f ${local_dir}/wso2bam-${version}.zip",
                                	   },
		}
	}
	...
	define initialize ( $repo, $version, $service, $local_dir, $target, $mode, $owner ) {
		...
		exec {  "creating_target_for_${name}":
			...
			command		=> "mkdir -p ${local_dir}";
	   		...
	   		"downloading_wso2bam-${version}.zip_for_${name}":
			path    	=> ["/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"],
			cwd		=> $local_dir,
			unless		=> "test -f ${local_dir}/wso2bam-${version}.zip",
			command		=> "wget -q ${repo}/wso2bam-${version}.zip",
			logoutput 	=> "on_failure",
			creates		=> "${local_dir}/wso2bam-${version}.zip",
			timeout 	=> 0,
			require		=> Exec["creating_local_package_repo_for_${name}",
						"creating_target_for_${name}"];
		
			"extracting_wso2bam-${version}.zip_for_${name}":
			path    	=> ["/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"],
			cwd		=> $target,
			unless		=> "test -d ${target}/wso2bam-${version}/repository",
			command 	=> "unzip ${local_dir}/wso2bam-${version}.zip",
			logoutput       => "on_failure",
			creates		=> "${target}/wso2bam-${version}/repository",
                        timeout 	=> 0,        
                        require 	=> Exec["downloading_wso2bam-${version}.zip_for_${name}"];

			"setting_permission_for_${name}":
			path            => ["/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"],
                        cwd             => $target,
			command         => "chown -R ${owner}:${owner} ${target}/wso2bam-${version} ;
					    chmod -R 755 ${target}/wso2bam-${version}",
			logoutput       => "on_failure",
			timeout         => 0,
                        require 	=> Exec["extracting_wso2bam-${version}.zip_for_${name}"];
		}
	}
		...
		file {  $target:
			...
			ignore		=> ".svn",
	                ...
	                recurse         => true;
                       ...
                       "$target/repository/components/patches/":
                        owner           => $owner,
                        group           => $group,
                        source          => ["puppet:///modules/common/patches/",
	                ...
	                "$target/repository/components/lib/":
                        owner           => $owner,
                        group           => $group,
                        source          => ["puppet:///modules/common/lib/",
                                            "puppet:///modules/${service}/lib/"],
                        sourceselect    => all,
                        ignore          => ".svn",
                        ensure          => present,
                        recurse         => true;
