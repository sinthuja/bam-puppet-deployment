class setup::bam_receiver_analyzer ( $version, 
		   $offset=0, 
		   $cluster_port=4000, 
	...
	$service_code 		= "bam_receiver_analyzer"
        $registry_database      = "bam_reg_db"
	$carbon_home		= "${target}/wso2${deployment_code}-${setup_version}"

	$service_templates 	=  ["conf/etc/tasks-config.xml",
                                    "conf/advanced/hive-site.xml", "conf/advanced/streamdefn.xml", "conf/data-bridge/data-bridge-config.xml"]
